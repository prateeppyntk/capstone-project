...
try:
    provinces_spark_df.write.format("delta").partitionBy("province_id").saveAsTable("default.provinces")
except:
    pass
...
sheet_pm_25_df = read_google_sheets(service, SAMPLE_SPREADSHEET_ID, "pm2_5!A:C")
...
sheet_pm_2_spark_df = spark.createDataFrame(sheet_pm_25_df)
levels_spark_df = spark.createDataFrame(levels_df)

pm_25 = sheet_pm_2_spark_df.union(pm_25)
...
pm_25 = pm_25.withColumn("date", to_date(col("date"), "dd-MM-yyyy"))
pm_25.write.format("delta").mode("overwrite").partitionBy("date").saveAsTable("default.pm2_5")

# Overwrite 'provincefactories' Delta table with new data and merge schema
burned_areas_spark_df.write.format("delta").mode("overwrite").partitionBy("year").saveAsTable("default.burnedareas")
province_factories_spark_df.write.format("delta").mode("overwrite").partitionBy("province_id").saveAsTable("default.provincefactories")

#create table first
try:
    region_spark_df.write.format("delta").partitionBy("region_id").saveAsTable("default.regions")
except:
    pass
try:
    factory_types_spark_df.write.format("delta").partitionBy("factory_type_id").saveAsTable("default.factorytypes")
except:
    pass
try:
    df_stations.write.format("delta").partitionBy("station_code").saveAsTable("default.stations")
except:
    pass
try:
    levels_spark_df.write.format("delta").partitionBy("Level").saveAsTable("default.levels")
except:
    pass
...
)SELECT * FROM v_pm2_5bystation cluster by station
"""
...
, v_date AS (
  ...
  FROM 
    date_range_expanded )
SELECT * FROM v_date cluster by date
"""

# code create view v_pm2_5
create_view_v_pm2_5 = """Create view v_pm2_5 as 
SELECT p.station_code, s.province_id, p.date, p.value, l.Level as level, pr.region_id
...
LEFT JOIN default.provinces pr ON s.province_id = pr.province_id
cluster by province_id
"""

# code create view v_burnedareas01
create_view_v_burnedareas = """Create view v_burnedareas as 
SELECT *, CONCAT(CASE 
                WHEN year > 2500 THEN year - 543
                ELSE year
            END, '-1-1') AS date_string
FROM default.burnedareas cluster by year, province_id
"""
